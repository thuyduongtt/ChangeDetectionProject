...
class Dataset(object):
    ...
    def init_from_stable_datasets(self):
        ...
        self.datasetInstance = DatasetInstance_OurAerial.DatasetInstance_OurAerial(self.settings, self.dataLoader,
                                                                                   dataset_variant)
        ...
        self.train, self.val, self.test = self.datasetInstance.split_train_val_test_KFOLDCROSSVAL(self.data,
                                                                                                  test_fold=test_fold,
                                                                                                  K=K)
        self.paths = np.asarray(self.paths)
        self.train_paths, self.val_paths, self.test_paths = self.datasetInstance.split_train_val_test_KFOLDCROSSVAL(
            self.paths, test_fold=test_fold, K=K)

        print("Has ", len(self.train[0]), "train, ", len(self.val[0]), "val, ", len(self.test[0]), "test, ")
        print("Has ", len(self.train_paths[0]), "train_paths, ", len(self.val_paths[0]), "val_paths, ",
              len(self.test_paths[0]), "test_paths, ")
